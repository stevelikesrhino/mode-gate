...
function stripHashlinePrefix(line: string): string {
	const match = line.match(/^\s*(?:>>>|>>)?\s*[+-]?\s*\d+\s*#\s*[ZPMQVRWSNKTXJBYH]{2}:(.*)$/);
	return match ? match[1] : line;
}

function stripHashlinePrefixes(lines: string[]): string[] {
	return lines.map(stripHashlinePrefix);
}

function splitEmbeddedNewlines(lines: string[]): string[] {
	return lines.flatMap((line) => normalizeLineEndings(line).split("\n"));
}

function parseEditLines(lines: string[]): string[] {
	return stripHashlinePrefixes(splitEmbeddedNewlines(lines));
}

function parseInsertLines(lines: string[]): string[] {
	return lines.length === 0 ? [""] : lines;
}

export function parseRawEdits(
	rawEdits: Array<{ op: string; pos: string; end?: string; lines: string[] }>,
	_filePath: string,
): HashlineEdit[] {
	return rawEdits.map((raw) => {
		switch (raw.op) {
			case "replace": {
				...
				return { op: "replace", pos: parseTag(raw.pos), end: parseTag(raw.end), lines: parseEditLines(raw.lines) };
			}
			case "insert_after": {
				const lines = parseInsertLines(parseEditLines(raw.lines));
...
function getEditSortPosition(edit: HashlineEdit): number {
	switch (edit.op) {
		case "insert_before":
			return edit.pos.line - 0.25;
		case "replace":
...
function compareEditsByDisplayOrder(a: HashlineEdit, b: HashlineEdit): number {
	const left = getEditRange(a);
	const right = getEditRange(b);
	return getEditSortPosition(a) - getEditSortPosition(b) || left.end - right.end;
}

function getEditLineDelta(edit: HashlineEdit): number {
	switch (edit.op) {
		case "replace":
			return edit.lines.length - (edit.end.line - edit.pos.line + 1);
		case "insert_before":
		case "insert_after":
			return edit.lines.length;
	}
}

function getAddedStartLines(edits: HashlineEdit[]): Map<HashlineEdit, number> {
	const starts = new Map<HashlineEdit, number>();
	let lineDelta = 0;

	for (const edit of [...edits].sort(compareEditsByDisplayOrder)) {
		switch (edit.op) {
			case "replace":
			case "insert_before":
				starts.set(edit, edit.pos.line + lineDelta);
				break;
			case "insert_after":
				starts.set(edit, edit.pos.line + lineDelta + 1);
				break;
		}
...
function buildEditDiffHunks(edits: HashlineEdit[], oldLineCount: number, contextLines: number): EditDiffHunk[] {
	const sorted = [...edits].sort(compareEditsByDisplayOrder);
...
export function generateEditDiff(
	...
	const addedStartLines = getAddedStartLines(edits);
	const output: string[] = [];
	...
	for (const hunk of buildEditDiffHunks(edits, oldDoc.lines.length, contextLines)) {
		...
		for (const edit of hunk.edits) {
			...
			if (edit.op === "insert_after") {
				...
				appendAddedLines(output, addedStartLines.get(edit) ?? edit.pos.line + 1, edit.lines, lineNumWidth);
			...
			if (edit.op === "insert_before") {
				appendAddedLines(output, addedStartLines.get(edit) ?? edit.pos.line, edit.lines, lineNumWidth);
