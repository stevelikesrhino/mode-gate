/**
 * Hashline editing tools registered by the mode-gate extension.
 *
 ...
 *
 * Overrides: `read` (hashline-formatted output) and registers `line-edit` (anchor-based operations).
 */
...
async function computeEditPreview(
	path: string,
	edits: Array<{ op: string; pos?: string; end?: string; lines: string[] }>,
	cwd: string,
	...
	edits: Type.Array(
		Type.Object({
			op: StringEnum(
				...
				{
					description:
						"replace: Replace lines from pos to end (inclusive). Use the same anchor for pos and end to replace a single line. Use empty lines [] to delete the range. " +
			...
			lines: Type.Array(Type.String(), {
				description: "Replacement or insertion lines. Each string is exactly one logical file line; never include \\n or \\r inside a string. Raw file text only; no LINE#HASH prefixes. Use [] to delete with replace. Use [\"\"] to insert or replace with a blank line.",
			}),
		}),
	),
});
...
export function registerLineEditTools(pi: ExtensionAPI, settings = loadModeGateSettings()): void {
	...
	// ─── Add read-image (delegate to system read) ─────────────────────────
	const readImageTool = defineTool({
		...
		promptGuidelines: ["Use read-image for image files to preserve default image attachment behavior."],
	...
	const readTool = defineTool({
		...
		description:
			...
			"Use these LINE#HASH anchors when calling the line-edit tool.",
		...
		promptGuidelines: [
			...
			"The LINE#HASH anchors are used by the line-edit tool to identify lines.",
	...
	const lineEditTool = defineTool({
		...
		description:
			...
			"Always read first. Use exact LINE#HASH anchors. Never include anchors in lines.",
		...
		renderCall(args, theme, context) {
			...
			const text = (context.lastComponent as Text | undefined) ?? new Text("", 0, 0);
			let content = `${theme.fg("toolTitle", theme.bold("line-edit"))} ${theme.fg("accent", args?.path)}`;
