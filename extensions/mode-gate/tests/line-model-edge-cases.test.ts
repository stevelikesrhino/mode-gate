...
function replaceLineWith(content: string, line: number, oldLine: string, replacement: string): string {
	...
	const edits = parseRawEdits(
		[{ op: "replace", pos: anchor, end: anchor, lines: linesOf(replacement) }],
		"edge-case.txt",
	);
...
function insertAfter(content: string, line: number, oldLine: string, insertion: string): string {
	const edits = parseRawEdits(
		[{ op: "insert_after", pos: formatLineTag(line, oldLine), lines: linesOf(insertion) }],
		"edge-case.txt",
	);
	return applyHashlineEdits(content, edits).result;
}
...
function insertBefore(content: string, line: number, oldLine: string, insertion: string): string {
	const edits = parseRawEdits(
		[{ op: "insert_before", pos: formatLineTag(line, oldLine), lines: linesOf(insertion) }],
		"edge-case.txt",
	);
	return applyHashlineEdits(content, edits).result;
}
...
test("multiple edits apply bottom-up without shifting earlier anchors", () => {
	const first = { op: "replace", pos: formatLineTag(1, "A"), end: formatLineTag(1, "A"), lines: ["AA"] };
	const second = { op: "replace", pos: formatLineTag(3, "C"), end: formatLineTag(3, "C"), lines: ["CC"] };
...
test("edit lines strip accidental hashline prefixes", () => {
	const edits = parseRawEdits(
		[{
			op: "replace",
			pos: formatLineTag(1, "old"),
			end: formatLineTag(1, "old"),
			lines: ["1#TX:new", ">>> 2#VR:  indented", "+ 3#PM:plus-marked"],
		}],
		"edge-case.txt",
	);
	...
	assert.deepEqual(edits[0], {
		op: "replace",
		pos: { line: 1, hash: formatLineTag(1, "old").split("#")[1] },
		end: { line: 1, hash: formatLineTag(1, "old").split("#")[1] },
		lines: ["new", "  indented", "plus-marked"],
	});
});

test("edit lines split accidental embedded newlines before diff rendering", () => {
	const oldContent = "A\nB\nC";
	const rawEdits = [
		{
			op: "replace",
			pos: formatLineTag(2, "B"),
			end: formatLineTag(2, "B"),
			lines: ["BB\nCC\nDD"],
		},
	];
	const edits = parseRawEdits(rawEdits, "edge-case.txt");
	const newContent = applyHashlineEdits(oldContent, edits).result;

	assert.equal(newContent, "A\nBB\nCC\nDD\nC");
	assert.equal(
		generateEditDiff(oldContent, edits, newContent, 1).diff,
		[
			" 1 A",
			"-2 B",
			"+2 BB",
...
test("edit diff renders unlimited anchored edits without sliding repeated blank lines", () => {
	...
	const rawEdits = [
		{ op: "replace", pos: formatLineTag(1, "aaa"), end: formatLineTag(1, "aaa"), lines: ["LINE1!!!"] },
		{ op: "replace", pos: formatLineTag(50, ""), end: formatLineTag(50, ""), lines: ["LINE50!!!"] },
		{ op: "replace", pos: formatLineTag(100, ""), end: formatLineTag(100, ""), lines: ["LINE100!!!"] },
		{ op: "replace", pos: formatLineTag(150, ""), end: formatLineTag(150, ""), lines: ["LINE150!!!"] },
		{ op: "replace", pos: formatLineTag(200, oldLines[199]), end: formatLineTag(200, oldLines[199]), lines: [] },
	];
...
test("edit diff numbers later additions after earlier inserts in new-file coordinates", () => {
	const oldContent = "A\nB\nC";
	const rawEdits = [
		{ op: "insert_after", pos: formatLineTag(1, "A"), end: formatLineTag(1, "A"), lines: ["X"] },
		{ op: "replace", pos: formatLineTag(3, "C"), end: formatLineTag(3, "C"), lines: ["CC"] },
	];
	const edits = parseRawEdits(rawEdits, "TEST.md");
	const newContent = applyHashlineEdits(oldContent, edits).result;
...
test("edit diff numbers later additions after earlier deletions in new-file coordinates", () => {
	const oldContent = "A\nB\nC\nD";
	const rawEdits = [
		{ op: "replace", pos: formatLineTag(2, "B"), end: formatLineTag(2, "B"), lines: [] },
		{ op: "replace", pos: formatLineTag(4, "D"), end: formatLineTag(4, "D"), lines: ["DD"] },
	];
	const edits = parseRawEdits(rawEdits, "TEST.md");
	const newContent = applyHashlineEdits(oldContent, edits).result;
