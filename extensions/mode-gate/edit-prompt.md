...
```
{"path":"src/app.ts","edits":[{"op":"replace","pos":"6#JB","end":"6#JB","lines":["  return `hello ${displayName}${punctuation}`;"]}]}
```

## Example multi-line replace using lines 4-6 from the snippet:
```
{"path":"src/app.ts","edits":[{"op":"replace","pos":"4#PM","end":"6#JB","lines":["  const displayName = capitalize(name.trim());","  const punctuation = excited ? \"!\" : \".\";","  return `hello ${displayName}${punctuation}`;"]}]}
```

## Example insert_after using line 4 from the snippet:
```
{"path":"src/app.ts","edits":[{"op":"insert_after","pos":"4#PM","end":"4#PM","lines":["  if (!displayName) return \"hello.\";"]}]}
```

## Example insert_before using line 1 from the snippet:
```
{"path":"src/app.ts","edits":[{"op":"insert_before","pos":"1#VR","end":"1#VR","lines":["import { logger } from \"./logger\";"]}]}
```

## Example delete using lines 4-5 from the snippet:
```
{"path":"src/app.ts","edits":[{"op":"replace","pos":"4#PM","end":"5#NQ","lines":[]}]}
```
...
- Never include LINE#HASH: prefixes in lines.
- Each item in lines is exactly one logical file line. Never include \n or \r inside a lines item.
- Make the minimum exact edit. Do not rewrite, reformat, or clean up unrelated code.
- lines must use matching indentation. If the file uses tabs, use real tabs.
...
- When replacement lines end with a closing delimiter like }, */, ), or ], verify end includes the original line carrying that delimiter.
